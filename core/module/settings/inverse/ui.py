...
class Inverse(BaseSettingsPanel):
    ...
    def configure(self, column_index, caller_index=None):
        ...
        self.elements['title'].label.setText("Inverse " + str(data.df.columns[column_index]))
    ...
    def inverse_handler(self):
        column_name = data.df.columns[self.column_index]
        data.df[column_name] = data.df[column_name].max() - (data.df[column_name]-data.df[column_name].min())
        self.root_class.action_update_data_panel()
