...
_font_regular = QtGui.QFont(_FONT_FAMILY)
_font_regular.setPointSize(12)

_font_result_label = QtGui.QFont(_FONT_FAMILY)
_font_result_label.setPointSize(16)

# Smaller, bold study title shown in the main area (brand-colored).
_font_study_title = QtGui.QFont(_FONT_FAMILY)
_font_study_title.setPointSize(12)
_font_study_title.setBold(True)

_font_result_element_label = QtGui.QFont(_FONT_FAMILY)
_font_result_element_label.setPointSize(10)


# Active UI color scheme -- the single source of truth for widget colors. Its attributes are
# populated (below) from the active palette in src/common/ui_theme.py (LIGHT or DARK).
# Style.Color / Style.General reference these names, and every widget references Style.Color, so
# colors are not hardcoded anywhere else. To switch the whole UI light/dark, change ACTIVE_THEME
# in src/common/ui_theme.py (takes effect on the next start).
class Scheme:
    pass
...
class Style:
    ...
    class Color(Enum):
        ...
        # Semantic tokens -- every value references Scheme (the single color source above).
        # Widgets use these tokens; never hardcode a color in a widget.
    ...
    class General(Enum):
        ...
        # Complete borders (colors reference Scheme)
