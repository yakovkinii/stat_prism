...
from src.modules.ordnallogregression.ordnallogregression_result import OrdnalLogRegressionResult
from src.result_display_panel.result_widget_containers.html_widget_container import HTMLResultElement


def calculate_ordnallogregression(df, col1, col2):
    ordnallogregression = LinearRegression()
    ordnallogregression.fit(df[col1].values.reshape(-1, 1), df[col2].values.reshape(-1, 1))
    a = ordnallogregression.intercept_
    b = ordnallogregression.coef_[0]
    return a, b


def recalculate_ordnallogregression_study(
    df: pd.DataFrame, result: OrdnalLogRegressionResult, filter_result: Union[FilterResult, None]
) -> OrdnalLogRegressionResult:
    logging.info("Recalculating ordnallogregression study")
    ...
    a, b = calculate_ordnallogregression(df, columns[0], columns[1])

    html_result_element = HTMLResultElement()
    html_result_element.items.append(HTMLText(f"linear chototam ordallogregression a={a}, b={b}"))
