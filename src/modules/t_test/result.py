...
<div>
    ...
    <ul>
        <li> The values in the Variable(s) columns are used to calculate the t-test.
        <li> The groups are determined by the values in the Grouping Column,
        which should have exactly two unique values.
    </ul>
</div>
...
<div>
    <ul>
        <li> The data is normally distributed.
        <li> The groups have equal variances.
        <li> The groups are independent.
    </ul>
</div>
...
class TTestResult(BaseResult):
    def __init__(self, unique_id, settings_panel_index, title, title_context, config: TTestStudyConfig):
