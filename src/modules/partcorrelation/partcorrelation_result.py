...
class PartCorrelationStudyConfig:
    def __init__(
        self,
...
class PartCorrelationResult(BaseResult):
    def __init__(
        self, unique_id, settings_panel_index, title=None, title_context=None, config: PartCorrelationStudyConfig = None
    ):
        ...
        if title is None:
            title = f"PartCorrelation, id={unique_id}"
        self.title = title
