...
from src.modules.partcorrelation.partcorrelation_result import PartCorrelationResult
...
def calculate_partcorrelation(df, col1, col2):
    partcorrelation = LinearRegression()
    partcorrelation.fit(df[col1].values.reshape(-1, 1), df[col2].values.reshape(-1, 1))
    a = partcorrelation.intercept_
    b = partcorrelation.coef_[0]
    return a, b


def recalculate_partcorrelation_study(
    df: pd.DataFrame, result: PartCorrelationResult, filter_result: Union[FilterResult, None]
) -> PartCorrelationResult:
    logging.info("Recalculating partcorrelation study")
    ...
    a, b = calculate_partcorrelation(df, columns[0], columns[1])

    html_result_element = HTMLResultElement()
    html_result_element.items.append(HTMLText(f"linear chototam partcorrelation a={a}, b={b}"))
