...
def get_statement_multiple_items(r, p, df, var1, var2, id1, id2, id3):
    if p > 0.05:
        return [
            f"The cross-correlation between '{var1}' and '{var2}' was found to be "
            f"non-significant, {format_apa(r, p, df)}. ",
            f"The results indicated that the relationship between the variables '{var1}' and '{var2}' "
            f"was not significant, {format_apa(r, p, df)}. ",
            f"There was no significant cross-correlation identified between '{var1}' and '{var2}', "
            f"{format_apa(r, p, df)}. ",
        ][id1.get() % 3]
    else:
        text = [
            f"There was a {get_strength(r)} {get_sign(r)} cross-correlation between the the "
            f"variables '{var1}' and '{var2}', {format_apa(r, p, df)}. ",
            f"A {get_strength(r)} and {get_sign(r)} cross-correlation was observed between '{var1}' and '{var2}',"
            f" {format_apa(r, p, df)}. ",
            f"The variables '{var1}' and '{var2}' showed a {get_strength(r)} and {get_sign(r)} cross-correlation, "
            f"{format_apa(r, p, df)}. ",
        ][id2.get() % 3]
...
def get_report(
    rows, columns, correlation_matrix, p_matrix, df_matrix, report_non_significant, kind: CrossCorrelationType
):
    id1 = ID()
    id2 = ID()
    id3 = ID()
    if kind == CrossCorrelationType.PEARSON:
        correlation_name = "Pearson cross-correlation coefficient"
    elif kind == CrossCorrelationType.SPEARMAN:
        correlation_name = "Spearman rank cross-correlation coefficient"
    elif kind == CrossCorrelationType.KENDALL:
        correlation_name = "Kendall rank cross-correlation coefficient"
    else:
    ...
    if p_matrix.min().min() > 0.05 and not report_non_significant:
        return "No significant cross-correlations were found between the mentioned variables."
